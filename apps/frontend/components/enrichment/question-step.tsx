...
export function QuestionStep({
  ...
  return (
    <div className="flex flex-col h-full min-h-[500px]">
      ...
      {item && (
        <div className="mb-6">
          <div className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 bg-[var(--surface-muted)] border border-[color:var(--border)] text-sm">
            {item.item_type === 'experience' ? (
              <Briefcase className="w-4 h-4 text-[color:var(--text-subtle)]" />
            ) : (
              <FolderKanban className="w-4 h-4 text-[color:var(--text-subtle)]" />
            )}
            ...
            {item.subtitle && (
              <span className="text-[color:var(--text-subtle)]">@ {item.subtitle}</span>
            )}
          </div>
        </div>
      )}
