...
const ResumeBuilderContent = () => {
  ...
  const handleApplyMissingKeywords = useCallback(
    (missingKeywords: string[], mode: 'skills-only' | 'skills-and-summary') => {
      ...
      if (!result.addedSkillCount) {
        showNotification(t('builder.jdMatch.applyNoChanges'), 'warning');
        return;
      }

      handleUpdate(result.resumeData);
      setActiveTab('resume');
      showNotification(
        t('builder.jdMatch.applySuccess', { count: result.addedSkillCount }),
        'success'
      );
    },
    [handleUpdate, resumeData, showNotification, t]
  );
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {activeTab === 'jd-match' && jobDescription && (
                <JDComparisonView
                  ...
                  onApplyMissingKeywords={handleApplyMissingKeywords}
                />
              )}
            </div>
          </div>
