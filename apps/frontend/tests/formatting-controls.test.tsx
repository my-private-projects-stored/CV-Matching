...
describe('FormattingControls', () => {
  ...
  it('renders the default formatting summary', () => {
    ...
    expect(
      screen.getByText(
        (_, element) => element?.textContent === 'builder.formatting.effectiveSectionGap: 1rem'
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (_, element) => element?.textContent === 'builder.formatting.effectiveItemGap: 0.25rem'
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (_, element) => element?.textContent === 'builder.formatting.effectiveLineHeight: 1.35'
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (_, element) => element?.textContent === 'builder.formatting.effectiveBaseFont: 14px'
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (_, element) => element?.textContent === 'builder.formatting.effectiveHeaderScale: 2x'
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
    ...
    expect(
      screen.getByText(
        (_, element) =>
          element?.textContent ===
          'builder.formatting.effectiveBodyFont: builder.formatting.fontNames.sans'
      )
    ).toBeInTheDocument();
  });
  ...
  it('updates the main formatting controls and resets cleanly', () => {
    ...
    fireEvent.click(
      screen
        .getByText('builder.formatting.spacingSection:')
        .parentElement!.querySelectorAll('button')[4]
    );
    expect(onChange).toHaveBeenLastCalledWith(
    ...
    fireEvent.click(
      screen.getAllByRole('button', { name: 'builder.formatting.fontNames.mono' })[0]
    );
    expect(onChange).toHaveBeenLastCalledWith(
    ...
    fireEvent.click(
      screen.getAllByRole('button', { name: 'builder.formatting.fontNames.mono' })[1]
    );
    expect(onChange).toHaveBeenLastCalledWith(
    ...
    fireEvent.click(
      screen.getByText('builder.formatting.compactMode').closest('label')!.querySelector('button')!
    );
    expect(onChange).toHaveBeenLastCalledWith(expect.objectContaining({ compactMode: true }));

    fireEvent.click(
      screen.getByText('builder.formatting.contactIcons').closest('label')!.querySelector('button')!
    );
    expect(onChange).toHaveBeenLastCalledWith(expect.objectContaining({ showContactIcons: true }));

    fireEvent.click(
      screen.getByRole('button', { name: 'builder.formatting.templates.modern.name' })
    );
    expect(onChange).toHaveBeenLastCalledWith(expect.objectContaining({ template: 'modern' }));
    ...
    expect(onChange).toHaveBeenLastCalledWith(expect.objectContaining({ accentColor: 'green' }));
  ...
  it('updates effective output when compact mode is enabled', () => {
    ...
    expect(
      screen.getByText(
        (_, element) => element?.textContent === 'builder.formatting.effectiveSectionGap: 0.6rem'
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (_, element) => element?.textContent === 'builder.formatting.effectiveItemGap: 0.15rem'
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        (_, element) => element?.textContent === 'builder.formatting.effectiveLineHeight: 1.24'
      )
    ).toBeInTheDocument();
