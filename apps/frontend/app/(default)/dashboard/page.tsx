...
export default function DashboardPage() {
  ...
  const [settingMasterResumeId, setSettingMasterResumeId] = useState<string | null>(null);
  ...
  const handleSetAsMaster = async (e: React.MouseEvent, resumeId: string) => {
    e.stopPropagation();
    if (!resumeId || settingMasterResumeId === resumeId || resumeId === masterResumeId) {
      return;
    }

    try {
      setSettingMasterResumeId(resumeId);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
        ...
        {tailoredResumes.map((resume) => {
          ...
          const isSettingAsMaster = settingMasterResumeId === resume.resume_id;
          return (
            <Card
                  ...
                  >
                    {isSettingAsMaster ? (
                      <>
                        ...
                        {t('dashboard.settingAsMaster')}
                      </>
                    ) : (
                      t('dashboard.setAsMaster')
                    )}
                  </Button>
                </div>
              </div>
