...
export default function RecruiterNotificationsPage() {
  const header = usePageHeader('recruiterNotifications');
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && !error && items.length === 0 ? (
        <EmptyState
          title={t('emptyStates.noNotificationsRecruiter.title')}
          description={t('emptyStates.noNotificationsRecruiter.description')}
          action={
            <Link className="text-sm font-semibold text-[var(--blue-700)]" href="/recruiter/jobs">
              {t('recruiter.notifications.viewJobPostings')}
            </Link>
          }
        />
      ) : null}
