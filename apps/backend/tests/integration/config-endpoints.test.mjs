...
function getTestMongoUri() {
  ...
  url.pathname = `/${dbName}_integration`;
  return url.toString();
}
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const featureGet = await requestJson(baseUrl, "GET", "/config/features");
      ...
      const companyProfileGet = await requestJson(baseUrl, "GET", "/config/company-profile");
      ...
      const apiKeysAfterBlank = await requestJson(baseUrl, "GET", "/config/api-keys");
      ...
      const apiKeysGet = await requestJson(baseUrl, "GET", "/config/api-keys");
      ...
      const deleteProvider = await requestJson(baseUrl, "DELETE", "/config/api-keys/google");
      assert.equal(deleteProvider.status, 204);

      const clearAllMissingConfirm = await requestJson(baseUrl, "DELETE", "/config/api-keys");
